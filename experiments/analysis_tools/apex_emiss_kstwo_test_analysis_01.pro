...
pro apex_emiss_kstwo_test_analysis_01,in_names,$
  out_names,var_names, $
...
;Plot the temperature array  with no axes on the right
                plot,times[0:n_maps-1],t_result , $
                     ...
                     /NODATA, FONT=FONT,$
                     THICK=10, $
                     XRANGE=[min(times[0:n_maps-1]) ,$
                             max(times[0:n_maps-1])], XS=1
                OPLOT,times[0:n_maps-1],t_result ,  thick=8, COLOR=1
                
                axis, YAXIS=1, $
                      YRANGE=[.98*min(confidence),$
                         1.0*max(confidence)], $;[95, 100], $
                      YTITLE='% Confidence',$
                      /YSTYLE, $
                      CHARTHICK=1.5, CHARSIZE=1.5,$
                      /SAVE, FONT=FONT
                ...
                legend, ['D Stat.', '% Con.'],line=[0,0],$ ;PSYM=[4,4],$
