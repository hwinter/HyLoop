;
;This procedure produces a single plot that charts apex n_e as a function 
;  time with error bars .
;
pro   loop_ne_v_time_plot, $
...
if size(PLOT_PREFIX, /TYPE) ne 7 then $
  PREFIX='ne_v_time_' else  PREFIX=PLOT_PREFIX
...
;                      string(loop[0].state.time,format='(f9.2)')+' s'
            ...
            n_e_temp=moment(n_e, SDEV=SDEV_TEMP)
                             ;l loop
            n_e_error[file_index]=(SDEV_TEMP/])
...
;Plot the temperature array  with no axes on the right
        ...
        if not keyword_set(YRANGE) then $
          YRANGE_in=[.98*min(n_e-n_e_error),$
                     1.02*max(n_e+n_e_error)] else $
          YRANGE_in=YRANGE
        ...
        OPLOT,times,n_e ,  thick=8, COLOR=3

        errplot, n_e-n_e_error,n_e+n_e_error, COLOR=3
