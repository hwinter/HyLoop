;.r mk_xr_movie_02a_t_binned_med.pro
...
MAP1_NAME='12_25kev_7arc_avg.map'
...
prefix='image_2a_t_binned'
TITLE=['3-6 keV', $
       '3-6 keV NT ',$
       '3-6 keV Therm ' $
      ]
if not keyword_set(movie_name) then movie_name='avg_3_6_2a_t_binned'
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;Define coordinates
YC=0.0
XC=940.
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;   
;Cheat.  Ensure that the total signal is the sum of the two         
...
;         for l=0ul, min([n_elements(map1),n_elements(map2)])-1ul do begin
             ps_name=plots_dir+prefix+$
                     string(movie_counter,format='(I05)')+'.eps'
             gif_name=gif_dir+prefix+$
                      string(movie_counter,format='(I05)')+'.gif'
...
;Overplot the apex map
             a_map_dummy=a_map
             a_map_dummy.data>=1d-7
             plot_map, a_map_dummy,$          ;/notitle, $
                       /iso,/square,  $   ;_extra=gang_plot_pos(1,2,1), $
                       charthick=1.2, charsize=1.2,$
                       font=0 ,   background=255, $ 
...
;Bottom plot
             ...
             plot, times, (therm_signal+nt_signal), /nodata, $
                   ...
                   /noerase, /YLOG, yrange=[1, 1d5], ystyle=1,$
                   xrange=[0, 500], /XS
             oplot, times, (therm_signal+nt_signal), thick=5
             oplot, times, (therm_signal), line=2, color=1, thick=5
             oplot, times, (nt_signal), line=2, color=2, thick=5
...
;stop
         ...
         save, times, therm_signal, nt_signal, file='apex_3_6_kev_signal.sav'
