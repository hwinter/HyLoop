;compare each grid to the one before it
;With dist

pro  loop_grid_test1_t_n_e, $
...
if size(MULTI_FOLDERS, /TYPE) ne 7 then begin
    MESSAGE,' Procedure loop_grid_test2_t_n_e reqires the keyword MULTI_FOLDERS to be set.'
    stop
end
...
;Put all of the files in a variable array.
        ...
        time=lonarr(n_files)
        Diff_n_e_array=make_array(n_mfs-1ul,n_files, /DOUBLE, /NOZERO)
...
;Also identify the index of the loop apex.
            ...
            z=z_temp[0]
            min_index=where(abs(loop.s[z]-dist-loop.s) eq $
                                min(abs(loop.s[z]-dist-loop.s)))
            max_index=where(abs(loop.s[z]+dist-loop.s) eq $
                                min(abs(loop.s[z]+dist-loop.s)))
            n_elem=n_elements(loop.s[min_index[0]:max_index[0]])
            time=[time, loop.state.time]
            temp_temp=get_loop_temp(loop)
            temp[0,file_index ]= $
              total(temp_temp[min_index[0]:max_index[0]])/n_elem
            n_e[0,file_index ]=$
              total(loop.state.n_e[min_index[0]:max_index[0]])/n_elem
            ...
            for k=1ul, n_mfs-1ul do begin
                ...
                n_e_temp=(n_e[k, file_index]-n_e[k-1ul, file_index])$
                         /n_e[k, file_index]
                Diff_n_e_array[k-1ul,file_index]=n_e_temp[0]*100.
                t_temp=(temp[k, file_index]-temp[k-1ul, file_index])$
                       /temp[k, file_index]
                Diff_t_array[k-1ul, file_index]=t_temp[0]*100.
...
;Plot the temperature array  with no axes on the right
            ...
            OPLOT,X_VALS_in[1:*], Diff_t_array[*,file_index], THICK=8, COLOR=1
            OPLOT,X_VALS_in[1:*], Diff_t_array[*,file_index], SYMSIZE=10, COLOR=1, $
                   PSYM=4, /CLIP
            ...
            oplot,X_VALS_in[1:*], Diff_n_e_array[*,file_index],$
                  COLOR=3,THICK=4
            OPLOT,X_VALS_in[1:*], Diff_n_e_array[*,file_index], SYMSIZE=10, COLOR=3, $
                   PSYM=4, /CLIP
