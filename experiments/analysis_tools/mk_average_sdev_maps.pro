...
pro mk_average_sdev_maps,in_names, out_names, $
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;Here's where the Standard Deviations come in               
                for jjj=0ul,n_elements(map)-1 do begin    
                   data=reform(map0[jjj, *].data)
                   avg_data=get_array_mean(data, 2, STD_DEV=STD_DEV)                  
                   map_out[jjj].data=avg_data
                   map_sdev[jjj].data=STD_DEV
                endfor
                
                print, map_dir_main+out_names[m]
                map=map_out
                save, map,file=map_dir_main+out_names[m]
                break_file, map_dir_main+out_names[m], DISK_LOG, DIR, FILNAM, EXT
                dev_name= DISK_LOG+DIR+FILNAM+'_SDEV'+EXT
                map=map_sdev
                save, map,file=dev_name
                delvarx, map, map0, map00,map_sdev
