...
;  In this procedure a log-normal distribution is assumed and the distribution
;  is transformed to a normal distribution.
; apex_emiss_compare3.pro
;
pro  apex_emiss_compare4, map_name ,$
  TITLE=TITLE,$
...
;Apex submap
            ...
            for ll=0,N_files -1ul do begin
                Apex_emission[ll, k, i]=alog(abs(total(a_map[ll].data) ))
...
;        Apex_emiss_avg[ll,i]=exp(junk[0]-((junk[1])/2))
...
i=0
for ll=0ul, n_files-1ul do begin
    test=grubbs(Apex_emission[ll, *,0], crit=crit)
test=0
    if test eq 1 then begin
        index=where(abs(Apex_emission[ll, *,0]) le abs(crit))
        help, index
        if index[0] ne -1 then $ 
          emiss1=abs(Apex_emission[ll,index ,0]) else $
                     emiss1=abs(Apex_emission[ll,* ,0])
    endif else emiss1=abs(Apex_emission[ll, *,0])
        avg_emiss1=abs(mean(emiss1))
        median_1=abs(median(emiss1))
endfor
...
i=1
for ll=0ul, n_files-1ul do begin
    test=grubbs(Apex_emission[ll, *,1], crit=crit)
test=0
    if test eq 1 then begin
        index=where(abs(Apex_emission[ll, *,1]) le abs(crit))
        help, index
        if index[0] ne -1 then $ 
          emiss2=abs(Apex_emission[ll,index ,1]) else $
                     emiss2=abs(Apex_emission[ll,* ,1])

    endif else emiss2=abs(Apex_emission[ll, *,1])
        avg_emiss2=abs(mean(emiss2))
        median_2=abs(median(emiss2))
...
;Make a plot      
            ...
            set_plot, 'z'
           ; junk=where(Apex_emission[ll, *,0] le 0)
           ; if junk[0] ne -1 then Apex_emission[ll, junk,0]=1d-31
           ; junk=where(Apex_emission[ll, *,1] le 0)
           ; if junk[0] ne -1 then Apex_emission[ll, junk,1]=1d-31
            
            plot_histo,emiss1,steps1, histo1
            plot_histo,emiss2,steps2, histo2
...
;Plot the temperature array  with no axes on the right
            ...
            OPLOT,(steps1),histo1 , thick=10, COLOR=1,$
                 PSYM=10
            OPLOT,[avg_emiss1,avg_emiss1 ],$
                  [yrange[0], yrange[1]] ,$
                  thick=10, COLOR=1,$
                   line=1   
            OPLOT,[median_1, median_1],$
                  [yrange[0], yrange[1]] ,$
                  thick=10, COLOR=1,$
                  PSYM=4;, line=1   
            ...
            OPLOT,steps2,histo2 , thick=10, COLOR=3,$
                 PSYM=10
            OPLOT,[avg_emiss2,avg_emiss2 ],[yrange[0], yrange[1]] ,$
                  thick=10, COLOR=3,$
                  PSYM=10, line=1   
            OPLOT,[median_2, median_2],$
                  [yrange[0], yrange[1]] ,$
                  thick=10, COLOR=3,$
                  PSYM=4;, line=1   
