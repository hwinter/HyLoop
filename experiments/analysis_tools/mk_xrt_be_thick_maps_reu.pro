     ...
     for j=0ul, n_grids-1ul  do begin
        for k=0ul, n_runs-1ul do begin
           ...
           files=file_search(Current_folder,$
                             ...
                             COUNT=N_loops, $
                             /FULLY_QUALIFY_PATH)
           if n_loops eq 0 then goto, end_k_jump
  ;         n_loops<=10
           for l=0ul, N_loops-1ul do begin
              if files[l] eq '' then goto, end_l_jump
              restore, files[l]
              old_loop=loop
              loops=loop
            ...
            ;  if not keyword_set(RAD) then begin
                 ...
                 RAD=max(loop.rad)
    ;          endif else $
    ;             loop.rad=loop.rad*(RAD/max(loop.rad))
    ;          if not keyword_set(AREA) then begin
                 loop.a*=4
    ;             AREA=max(loop.a)
     ;         endif else $
     ;            loop.a=loop.a*(AREA/max(loop.a))
              old_loop=loop
