...
;Contour Map
MAP0_NAME='sxt_be.map'
;Plot windows
MAP1_NAME='sxt_thin_al.map'
MAP2_NAME='sxt_dag.map'
MAP3_NAME='sxt_thick_al.map';'al_med_avg.map'
;MAP1_NAME=MAP2_NAME
n_images=500
...
if not keyword_set(movie_name) then movie_name='sxt_04a_binned'
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;Define coordinates
YC=0.0
XC=940.
...
for i=0ul,  n_folders-1ul do begin
  ...
  ;  !p.multi=[0,2,2]
             ...
             restore, MAP0_NAME
             map0=map;time_bin_map(map,bin_size)
             temp_map=map0
             ...
             temp_map.xc=XC
             temp_map.yc=YC
             sub_map,temp_map,a_map0,xrange=apex_xr,yrange=apex_yr 
             sub_map, temp_map, fp_map0, xrange=fp_xr,yrange=fp_yr
            ; fp_map=map         ;[0]
             map0.xc=XC
             map0.yc=YC
             delvarx, map
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
             restore,  MAP1_NAME
             map1=map;time_bin_map(map,bin_size)
             map1.xc=XC
             map1.yc=YC       
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
             restore, MAP2_name
             MAP2=map;time_bin_map(map,bin_size)
             MAP2.xc=XC
             MAP2.yc=YC
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
             restore, MAP3_NAME
             MAP3=map;time_bin_map(map,bin_size)
...
;First image 
             ...
             plot_map, map1[l],$          ;/notitle, $
                      ...
                      charthick=1.2, charsize=1.2, $
                       ...
                       XTICKS=2,/NOLABELS , $
                      title=TITLE[1], last=map1[index_1];,/log
...
;Overplot the apex map
             plot_map, a_map,$          ;/notitle, $
                       ...
                       charthick=1.2, charsize=1.2,$
                       font=0 ,   background=0, $ 
                       /BORDER, BTHICK=4,BCOLOR=fsc_color('WHITE'), $;255, $
                       /OVER , COLOR=0, $
                       levels =[ 40.] ,$
                       /percent, /NO_DATA
...
;Second image     
             ...
             plot_map, map2[l],$          ;/notitle, $
                      ...
                      charthick=1.2, charsize=1.2, $
                       ...
                       XTICKS=2,/NOLABELS , $
                      title=TITLE[2],last=map2[index_2];,/log
...
;Third image  
             ...
             plot_map, map3[l],$          ;/notitle, $
                      ...
                      charthick=1.2, charsize=1.2, $
                       ...
                       XTICKS=2,  /NOLABELS ,$
                      title=TITLE[3], last=map3[index_3];,/log
...
;Overplot the footpoint map
             plot_map, fp_map_t,$          ;/notitle, $
                       ...
                       charthick=1.2, charsize=1.2,$
                       font=0 ,   background=0, $ 
                       /BORDER, BTHICK=4,BCOLOR=fsc_color('WHITE'), $
                       /OVER , COLOR=0, $
                       levels =[ 40.] ,$
                       /percent, /NO_DATA
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
;Bottom plot
             ...
             plot, times, (a_signal_1), /nodata, $
                   ...
                   XRANGE=[0, 450],$
                   YRange=[.9*min([a_signal_0[0:n_images]/fp_signal_0[0:n_images],$
                           ...
                           8.]
                  ;         1.3*max([a_signal_0[0:n_images]/fp_signal_0[0:n_images],$
                  ;                 a_signal_1[0:n_images]/fp_signal_1[0:n_images],$
                  ;                 a_signal_2[0:n_images]/fp_signal_2[0:n_images],$
                  ;                 a_signal_3[0:n_images]/fp_signal_3[0:n_images]]) ];,$
                               ;  /YLOG
             oplot, times,$
                    (a_signal_0[0:n_images])/fp_signal_0[0:n_images],$
                    color=3, thick=4 ;, psym=4
             oplot, times,$
                    (a_signal_1[0:n_images])/fp_signal_1[0:n_images],$
                    color=1, thick=4 ;, psym=4
             oplot, times, $
                    (a_signal_2[0:n_images])/fp_signal_2[0:n_images], $
                    color=2, thick=4
             oplot, times, $
                    (a_signal_3[0:n_images])/fp_signal_3[0:n_images],  $
                    color=0, thick=4 ;, psym=4
             oplot, [times[l], times[l]], line, thick=4

             legend, title, color=[3,1,2,0],box=0,$
                     thick=[4,4,4,4], /TOP,$; horizontal=1,$
                     line=[0,0,0,0],charthick=0.9, charsize=0.9,$
                      /HORIZ,font=0
...
save,  a_signal_0,a_signal_1,a_signal_2,a_signal_3,$
       fp_signal_0,fp_signal_1,fp_signal_2,fp_signal_3,$
       scl_0,scl_1,scl_2,scl_3,times,$
       file=movie_dir+'sxt_sig_vars.sav'
end
