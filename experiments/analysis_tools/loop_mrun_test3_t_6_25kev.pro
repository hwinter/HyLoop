...
;
pro   loop_mrun_test_apex_emiss, map_name,test_name $
...
if size(MULTI_FOLDERS, /TYPE) ne 7 then begin
    MESSAGE,' Procedure loop_mrun_test3_t_6_26kev reqires the keyword MULTI_FOLDERS to be set.'
    stop
end

if size(PLOT_PREFIX, /TYPE) ne 7 then $
  PREFIX='mrun_test3_6_25kev' else  PREFIX=PLOT_PREFIX

if size(MOVIE_NAME, /TYPE) ne 7 then $
  MOVIE_NAME=strcompress(STRING(BIN_DATE(), $  
                                Format='(A, I4, 5I2.2, A)'), $
                         /REMOVE)+'mrun3_test__6_25kev'
...
; and density change as a function of grid spacing        
            for file_index=0ul, n_mfs-1ul do begin
