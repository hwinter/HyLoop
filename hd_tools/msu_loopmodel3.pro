...
;-
pro msu_loopmodel3, loop, interval, $
...
;Save a snapshot of the present state.
    ...
    save, loop, loops,ENERGY_PD, PARTICLE_PD, $
          ...
          DELTA_MOMENTUM ,N_E_CHANGE,$
          FILENAME=strcompress(FILE_PREFIX $
          +string(loop.state.time,FORMAT='(f012.4)') $
          +'.'+FILE_EXT,/REMOVE_ALL), $
          /COMPRESS
...
;stop
    if not keyword_set(QUIET) then begin
       temps=get_loop_temp(loop)
        ...
        print, 'T min/max: '+string(min(temps))+'/'+string(max(temps))
        
        print,'grid spacing runs from ',min(ds)/100.0,' m  to ',max(ds)/100000.0,' km.'
        print, 'Heating function: '+HEAT_FUNCTION
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
if loop.t_max ge 1d9 then stop
