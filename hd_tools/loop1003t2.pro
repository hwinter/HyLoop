...
common loopmodel, dx1, dx2, A1, ix,sof
...
conduction=msu_t_conduction(state,A, T, x, t2,dt,ix,dx1,dx2, sof, SPIT=SPIT)

radiative_loss= - radloss_t2(state.n_e[1:ix-1],T[1:ix-1], $
	uri=uri,fal=fal,so=sof)*dt 

;viscous energy loss - mu is imported
 temp1 = 2.*(A[1:ix-1]*state.v[1:ix-1] - A[0:ix-2]*state.v[0:ix-2]) / $
	(dx1[0:ix-2]*(A[1:ix-1] + A[0:ix-2]))
 visc0 = dt * mu[0:ix] * [0d, (temp1)^2d0, 0d0]	;d^2 v heats the plasma
;ENDELSE
...
pro msu_loop, loop, heating, delta_t, debug=debug, $
              T0=T0, safety=safety, uri=uri,fal=fal,$
              spit=spit, novisc=novisc,SO=SO

;default boundary temperature
if not keyword_set(T0) then T0 = 1e4 

if not keyword_set(safety) then safety = 5.0

common loopmodel, dx1, dx2, A1, ix,sof
if keyword_set(SO) then sof=so
...
;Need to change the definition of the 
        ...
        mu = ( sqrt((kb*T)^5. *mp) *0.96*3d )/( coul_log*4d*sqrt(!pi)*qe^4. )
                                ;g/(cm s)
...
;calculate only once for all dt
        ne2=0.5*(state.n_e[0:is-1] + state.n_e[1:is])
...
;sub-diagonal elements of M
        main = mp*ne2 + [0.0, A[1:is-2]*(temp1 + temp2), 0.0]
...
;check for NaNs
    IF NOT finite(dt) THEN begin
        Print, 'Non-finite dt',string(dt)
        STOP                    ;check for NaNs
    endif
    dt0 = 0.6 * dt              ;a bit more than 1/2 timestep for stability
    
    ds0 = dstate(state, T, g, A, s, heating, dt0, T0,mu, $
                 debug, safety, uri, fal, spit)
...
;timestep scheme.
    ...
    ds = dstate(state0, T, g, A, s, heating, dt, T0,mu, $
                debug,  safety, uri, fal, spit)
