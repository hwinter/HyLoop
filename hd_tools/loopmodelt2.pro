...
for i=i1, num_iterations-1+i1 do begin
	...
	lhist(i) = state
        ...
        if (!d.name eq 'Z') then $ ;alternate means to monitor progress.
          save, state,g,A,x,heat, filename=outfile+'.snap' $
          else stateplot2, x, state, /screen
	...
	;	psym=3
        IF KEYWORD_SET(LOUD) THEN begin
            print,'Model time: '+$
                  strcompress(string(state.time),/REMOVE_ALL)+ $
                  's    Real time: '+$
                  strcompress(string(systime(1)-t_start),/REMOVE_ALL),'s'+ $
