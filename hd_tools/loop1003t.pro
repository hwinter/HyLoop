...
function radloss, n_e, T, uri=uri, fal=fal, disp=disp,so=so
...
function kappa, T, disp=disp, spit=spit,so=so
...
K2 = kappa(T2, spit=spit,so=sof) ;calculate thermal conductivity
...
pro loop1003t, state, g, A, x, heating, delta_t, debug=debug, $
