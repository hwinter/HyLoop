...
;============================================================================================================
PRO evolve10_t, infile, time, dt, ENERGY=ENERGY, outfile=outfile, q0=q0, $
...
;     s_form   = heating function in space, (erg/s/cm^3).  Must have
;     N-1 elements.  Defined in the volumes
;
 ...
 FOR i=0, n_do-1 DO BEGIN      ;MAIN LOOP
  ...
  ;IF version eq 1002 THEN $ ;older version of code, still has dA in K
   ; loop1002,state, g, A, x, hot[*,i], delta_t,uri=uri,fal=fal,spit=spit,debug=debug, novisc=novisc $
                                ;ELSE loop1003,state, g, A, x,
                                ;hot[*,i],
                                ;delta_t,uri=uri,fal=fal,spit=spit,debug=debug, novisc=novisc 

loop6,state, g, A, x, hot[*,i], delta_t,uri=uri,fal=fal,spit=spit,debug=debug, novisc=novisc
