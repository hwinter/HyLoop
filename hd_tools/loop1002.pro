...
;CALLING SEQUENCE:
;       loop1002, state, g, A, x, heating, delta_t $ 
;		[, /debug,/T0, /safety, /uri,/fal, /spit, /test ]
;	loop1002, state, g, A, x, heating, delta_t $
;		[, T0=T0, /uri, /fal, /debug, /test ]
;INPUT PARAMETERS:
...
function dstate, state, T, g, A, x, heating, dt, T0,mu, $
	 debug, safety, uri, fal, spit, test=test;, novisc=novisc
...
;IF keyword_set(test) OR keyword_set(novisc) THEN visc0=0. ELSE BEGIN
...
pro loop1002, state, g, A, x, heating, delta_t, debug=debug, $
	T0=T0, safety=safety, uri=uri,fal=fal, spit=spit, test=test, novisc=novisc
...
cs0=1e7	;not necessary if b.c. were in subroutine
...
while not done do begin
	...
	IF keyword_set(test) THEN BEGIN	;keep from dt=NaN problems
		T=state.e*0. 
		cs=cs0
		dt = min( dx1/(cs+ abs( state.v[0:ix-1] )) ) /safety
	ENDIF ELSE BEGIN
	...
	;calculate mu only once because T will not get updated
 		...
 		qe = 4.8d-10	;electron charge (statcoulombs= g^1/2 cm^3/2 /s)
	  	...
	  	IF keyword_set(test) THEN mu = fltarr(ix+1) + .5 ELSE $
	...
	;calculate only once for all dt
 		IF keyword_set(test) then ne2=1e9 ELSE $
	...
	state0.v = state.v + ds0.v + visc * (dt0/dt)
...
;plot, state.v
;oplot, state.v - ds.v -visc
;!p.multi=[0,2,1]
;plot, ds.v, yrange=2e4*[-1,1]
;plot, visc, linestyle=1, yrange=2e4*[-1,1]
;print, ds.v[where(state.v eq max(state.v))],$
;	visc[where(state.v eq max(state.v))]
endwhile
	state=bcs( state, g, T0)	;boundary conditions
