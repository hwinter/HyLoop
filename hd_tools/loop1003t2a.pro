...
;K2 = kappa_t2(T2, spit=spit,so=sof) ;calculate thermal conductivity
...
;conduction = 3*state.n_e*kB*(Tprime - T) ;used to use state.e instead of T,
	;but timestepping causes T and state.e to be out of sync!
...
pro loop1003t2, state, g, A, x, heating, delta_t, debug=debug, $
