...
;============================================================================================================
PRO evolve_test, g,a,x, time, dt, n_depth,e_h, $
...
; inname=strmid(infile, 0, strpos(infile,'.',/reverse_search))
 IF keyword_set(outfile) THEN savefile=outfile ELSE $
	savefile='~mcmullen/bp_sim/data/'+inname+'_'+strcompress(fix(time),/remove_all)+'.sav'
...
; shade_surf, [[e_h],[hot]]
 ...
 state=lhist(n_start)
; IF n_elements(computer) ne 0 THEN IF strupcase(computer) eq "MITHRA" THEN cd,'~/bp_sim/charles/' $
;	ELSE cd,'~/bp_sim/charles/earth/'	$;move to run c subroutines
;	ELSE cd,'~/bp_sim/charles/earth/'	;move to run c subroutines
 ...
 FOR i=0, n_do-1 DO BEGIN			;MAIN LOOP
  ...
  IF version eq 1002 THEN $	;older version of code, still has dA in K
   ...
   ELSE	loop1003t,state, g, A, x, hot[*,i], delta_t,uri=uri,fal=fal,spit=spit,$
