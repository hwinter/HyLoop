...
;============================================================================================================
PRO evolve_test_t2, g,a,x, time, dt,e_h, n_depth, $
...
if !d.name eq 'X' then begin
 if !d.window eq -1 then window,0,TITLE=!COMPUTER+' evolve_test_t2' $
   else wset,!d.window
endif
...
;Set the atomic physics parameters
 ...
 ENDIF ELSE BEGIN	;--- Kankelborgian Atomic Physics ---
	...
	uri = 1		;use Feldman abundances in radiative loss above 1e5K
...
;SET TIMES
 ...
 n_do=ulong(float(time)/float(delta_t) +0.5 )
 time=ulong(n_do*delta_t)				;reasign interger time to do
