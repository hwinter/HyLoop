...
;CALLING SEQUENCE:
;	loop6, state, g, A, x, heating, delta_t $
...
function radloss, n_e, T, T0=T0, src=src, uri=uri, fal=fal, disp=disp
...
log_t<=Tmax
;old, inefficient way
Lambda = 10^interpolate(logL, findex(logT,log_t ))
...
log_t>=Tmin
log_t<=Tmax
correction = 10^interpolate(logc, findex(logT,log_t ))
...
function dstate, state, T, g, A, x, heating, dt, T0, $
	src, debug, depth, safety, uri, fal
...
K2 = kappa(T2) ;calculate thermal conductivity
...
if keyword_set(depth) then begin
	x_alt = [2*x[0]-x[1],(x[0:ix-2]+x[1:ix-1])/2.0,2*x[ix-1]-x[ix-2]]
        	;x on the volume element grid (like e, n_e)
        L = x(ix-1) + x(0) ;Length of coronal loop
        
        ;find location of transition region at bottom of each loop leg.
	ss = where(T ge 1.1*T0)
        n_ss = n_elements(ss)
        i0 = ss(0)>1 & i1 = ss(n_ss-1)<(ix-3)
        
        strength = (1.0/safety)*((depth - x_alt[i0])/depth)^3 $
        ...
        strength = (1.0/safety)*((depth - (L - x_alt[i1]))/depth)^3 $
        	< 1.0/safety > (-1.0/safety) ;clamp density loss
        nscale = 0.5*min(state.n_e[i1:ix]) ;relevant density scale
        dn_e[i1] = dn_e[i1] + nscale*strength
	
	;message,'debug'
	;set_plot,'x'
	;wait,1.0
	;stateplot,x,state,/screen,/verbose
	;set_plot,'z'
...
pro loop6t2, state, g, A, x, heating, delta_t, debug=debug, $
...
;PRINT, 'dt: '+STRCOMPRESS(DT,/REMOVE_ALL)
