...
;-
pro loopeq, g, A, x, heat=heat, Ttop=Ttop, fname=fname, L=L, $
...
T0 = 1d4 ;loop base temperature (fixed)
...
if keyword_set(Ttop) then begin
	heat = 9.14e-7 * Ttop^3.51 * (L/2)^(-2)  ;scaling law
endif else begin
	Ttop = 52.6 * (L/2)^0.569 * heat^0.285 ;scaling law
endelse
...
loopmodel, g, A, x, heat, rtime, state=state, T0=1e4, $
	outfile=fname, /src, /fal, /uri, depth=depth, safety=safety
