...
;Insert the heating & momentum input from PaTC
    if keyword_set(NT_PART_FUNC) then begin
        if temp_loop.state.time lt patc_ticker then begin
            print, temp_loop.state.time,  patc_ticker
          goto, no_patc
          endif
...
;  injected n.t. beam.
        if  temp_loop.state.time gt !beam_switch_on_time $
          then $
...
;Here is the actual call to PATC
                patc, nt_beam,temp_loop[0],.1, DELTA_E=DELTA_E,$
...
;Convert to power [ergs/sec]              
                PATC_heating_rate=abs(-1d*DELTA_E*!msul_keV_2_ergs)/(0.1) ;(dt)   
...
;Save the Bremsstraulung emission information to an array.
                ...
                if size(NT_BREMS_array, /TYPE) eq 0 then $
