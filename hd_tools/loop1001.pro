...
;CALLING SEQUENCE:
;       loop1001, state, g, A, x, heating, delta_t $ 
...
function dstate, state, T, g, A, x, heating, dt, T0, $
	 debug, safety, uri, fal, spit, test=test

common loopmodel, dx1, dx2, A1, ix, cs0
...
;boundary conditions
;IF keyword_set(test) THEN state.v[0] = cs0 ELSE $
...
conduction=0.*fltarr(ix)	;TEST
radiative_loss = 0.		;TEST
IF NOT keyword_set(test) THEN BEGIN
 ...
 radiative_loss= - radloss(state.n_e[1:ix-1],T[1:ix-1],T0=T0,$
        	uri=uri,fal=fal)*dt 
ENDIF ;ELSE print,'test!'
	
;Round up all terms
de = conduction[1:ix-1] + advect_work   $
	+ radiative_loss $
	+ heating*dt
IF keyword_set(test) AND max(abs(de)) gt 1 THEN stop 
...
pro loop1001, state, g, A, x, heating, delta_t, debug=debug, $
	T0=T0, safety=safety, uri=uri,fal=fal, spit=spit, test=test
