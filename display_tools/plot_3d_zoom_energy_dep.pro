...
pro plot_3d_zoom_energy_dep, data_dir,$
  MOVIE_DIR=MOVIE_DIR, MOVIE_NAME=MOVIE_NAME,$
  FILE_PREFIX=FILE_PREFIX, FILE_EXT=FILE_EXT,$
  GIF_DIR=GIF_DIR

if size(data_dir, /TYPE) ne 7  then $
  data_dir='./'
if size(MOVIE_DIR, /TYPE) ne 7  then $
  movie_folder=data_dir+'/movies/'
if size(MOVIE_NAME, /TYPE) ne 7  then $
  movie_name=+MOVIE_DIR+'e_dep_tac_movie.gif'
if size(GIF_DIR, /TYPE) ne 7  then $
  GIF_DIR=Data_dir+'gifs/'

spawn, 'mkdir  '+movie_dir
FONT=1
OFFSET1=.5
if size(FILE_PREFIX, /TYPE) ne 7  then $
  file_prefix='patc*'
if size(FILE_EXT, /TYPE) ne 7  then $
  file_ext='.loop'
...
for frame=0, n_frames do begin
    temp_gif_name=gif_dir+gif_name_prefix+ $
                  string(frame,FORMAT= $  
                         '(I5.5)')+'.gif'
