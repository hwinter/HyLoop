...
;-
pro plot_avg_e_h, loop, fname=fname, screen=screen, verbose=verbose, $
                   LINESTYLE=LINESTYLE, WINDOW=WINDOW,$
                   XRANGE=XRANGE,EHRANGE=EHRANGE,$
...
if not keyword_set(TITLE) THEN TITLE='AVG E!DH!n Plot'
...
TYTITLE='E!DH!n (erg s!E-1!N cm!E-3!N)'
...
if not keyword_set(EHRANGE) then $
  EHRANGE=[min(T-std_t)*.9, max(T-std_t)*1.1]
...
;for i=0ul,n_loops-1ul do begin
    ...
    index=where(t_minus_error ge min(EHRANGE) and $
               t_plus_error le max(EHRANGE))
