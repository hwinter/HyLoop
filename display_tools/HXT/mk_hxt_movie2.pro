pro mk_hxt_movie,loop,INFILE=INFILE,OUTFILE=OUTFILE, GIF_DIR=GIF_DIR,$
...
M2_index_low=where(brems_struct[i].nt_brems[0].ph_energies $
                   le M2_range[0])
M1_index_low=M2_index_low[n_elements(M2_index_low)-1l]
...
scaleM2=255/max(image_struct.image_M2)
...
for i=0,n_elements(image_struct)-1 do begin
    ...
    image=[[image_struct.image_L],[image_struct.image_M1],[image_struct.image_M2]]
    TVMULTI,IMAGE,3,
    x2gif,gif_name
    gifs_L=[gifs_L,gif_name]
