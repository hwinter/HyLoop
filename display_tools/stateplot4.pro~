...
;-
pro stateplot4, loop, fname=fname, screen=screen, verbose=verbose, $
...
;for i=0ul,  n_loops-1ul do begin
if keyword_set(PLOT_LINE) then $
  oplot,   loop[0].s_alt,n_e,COLOR=color_array[i], $
           LINESTYLE=LINESTYLE, thick=LTHICK $
  else $
...
ERRPLOT, loop[0].s_alt,n_e-std_n_e, n_e+std_n_e, COLOR=color_array[i]
...
xyouts, extra.position[2],extra.position[3]*1.01, tstring,$
        FONT=FONT,/NORMAL,$
        CHARSIZE=CHARSIZ_INE, CHARTHICK=CHARTHICK_IN
