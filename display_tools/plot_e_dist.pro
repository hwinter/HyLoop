...
pro plot_e_dist, nt_beam, GIF_NAME=GIF_NAME,$
  ...
  if not keyword_set(N_RUNS) then n_runs_in=1.0 else $
     n_runs_in=N_RUNS
...
all_dead:
  ...
  n_part=total(in_beam.scale_factor)/n_runs_in
...
;ylog=1
  ...
  HISTOPLOT2, Double(in_beam.ke_total),energy_array ,n_array_T, BINSIZE=double(step),$
              ...
              DATACOLORNAME='black', font=0,TITLE=title, $
              CHARSIZE=charsize,CHARTHICK=charthick, $
              XTITLE=XTITLE_IN,YTITLE=YTITLE_IN,$
              /FILLPOLYGON, POLYCOLOR="red", $
