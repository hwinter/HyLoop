...
;-
pro stateplot4_temp, loop, loop2, fname=fname, screen=screen, verbose=verbose, $
...
temp=get_loops_avg_temp(loop,STD_DEV=std_temp)
if size(loop2, /TYPE) ne 0 then $
   temp2=get_loops_avg_temp(loop2,STD_DEV=std_temp2)
...
;if not keyword_set(DRANGE) then $
  DRANGE=[min(temp-std_temp)*.9,$
          max(temp+std_temp)*1.1]
print, 'DRANGE=',DRANGE
...
;endfor
YRANGE=[10^(5.5),10^(7.5d)]
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;Plot the Electron Density	
plot, loop[0].s_alt,temp, $
...
if size(loop2, /TYPE) ne 0 then  begin
    ...
    if keyword_set(PLOT_LINE) then $
      oplot,   loop2[0].s_alt,temp2 > yrange[0],COLOR=3, $;color_array[2], $
               LINESTYLE=LINESTYLE, thick=10 $ ;LTHICK $
      else $
      oplot,   loop2[0].s_alt,temp2 > yrange[0], psym=PSYM, $
               SYMSIZE=SYMSIZE, COLOR=3;color_array[2]

    ERRPLOT, loop2[0].s_alt,temp2-std_temp2 > yrange[0], temp2+std_temp2 > yrange[0], $
             COLOR=4;color_array[i]
 endif

;for i=0ul,  n_loops-1ul do begin
if keyword_set(PLOT_LINE) then $
  oplot,   loop[0].s_alt,temp > yrange[0],COLOR=1,$;color_array[i], $
           LINESTYLE=LINESTYLE, thick=LTHICK $
  else $
  oplot,   loop[0].s_alt,temp > yrange[0], psym=PSYM, $
           SYMSIZE=SYMSIZE, COLOR=1;color_array[i]

;endfor

ERRPLOT, loop[0].s_alt,temp-std_temp > yrange[0], temp+std_temp > yrange[0], COLOR=1;color_array[i]
