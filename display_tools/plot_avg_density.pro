...
;-
pro plot_avg_density, loop, fname=fname, screen=screen, verbose=verbose, $
                   LINESTYLE=LINESTYLE, WINDOW=WINDOW,$
                   XRANGE=XRANGE,DRANGE=DRANGE,$
                   FONT=FONT,XSIZE=XSIZE, YSIZE=YSIZE ,$
                   CHARSIZE=CHARSIZE, CHARTHICK=CHARTHICK,$
                   PLOT_LINE=PLOT_LINE, TITLE=TITLE, LOG=LOG,$
                   THICK=THICK, gpp=gpp, No_time=no_time, YTITLE=YTITLE


  if not keyword_set(Ytitle) then DYTITLE='N!De!N [cm!e-3!N]' else $
   DYTITLE=Ytitle
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
n_e=get_loops_avg_n_e(loop,STD_DEV=std_t)

if not keyword_set(DRANGE) then $
  DRANGE=[min(n_e-std_t)*.9, max(n_e-std_t)*1.1]
...
plot, loop[0].s_alt,n_e, $
     YTITLE=DYTITLE, $; xtitle='s (cm)', $
      LINESTYLE=LINESTYLE,  $
      XRANGE=XRANGE, YRANGE=DRANGE, $
      XS=1, FONT=FONT,  $
      ystyle=1,  $
...
;for i=0ul,n_loops-1ul do begin
    if keyword_set(PLOT_LINE) then $
      oplot, loop[0].s_alt,N_e,COLOR=colors[2], $
             LINESTYLE=LINESTYLE,THICK=thick*thick_factor  $
      else $
      oplot, loop[0].s_alt,n_e, psym=psym, $
             SYMSIZE=SYMSIZE*thick_factor, COLOR=colors[2]
    t_minus_error=n_e-std_T
    
    t_plus_error=n_e+std_T
    
    index=where(t_minus_error ge min(DRANGE) and $
               t_plus_error le max(DRANGE))
