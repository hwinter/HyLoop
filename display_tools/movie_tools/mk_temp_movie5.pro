pro mk_temp_movie5, folders,ref_folder, $
                   ...
                   MPEG=MPEG,LOOP=LOOP, $
                   TITLE=TITLE, frames_per_plot=frames_per_plot, $
                   MAX_N_FILES=MAX_N_FILES,$
                   VRANGE=VRANGE,$
                   XRANGE=XRANGE,TRANGE=TRANGE, $
                   DRANGE=DRANGE,PRANGE=PRANGE
...
IF NOT keyword_set(MOVIE_NAME) THEN MOVIE_NAME='temp_movie_avg'
...
animate_index=1
 for i=0ul, FILE_COUNT-1ul ,freq_out do begin
     ...
     IF  keyword_set(LOUD) then begin
         ...
         print,'########################################################################'
           ...
           index_string= string(animate_index,FORMAT= $  
                             '(I5.5)')
        POST=gif_dir+'temp_plot'+ $
             '_'+index_string+'.ps'
        gif_file=gif_dir+'temp_plot'+ $
                 '_'+index_string+'.gif'
        stateplot4_temp, loops,loops2, fname=POST,  verbose=verbose, $
     ...
     for j=0ul,frames_per_plot do begin
        gif_file0=gif_file
        post0=post
        index_string= string(animate_index,FORMAT= $  
                             '(I5.5)')
