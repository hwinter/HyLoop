...
pro  patc ,nt_particles,loop,duration, DELTA_E=DELTA_E,$
           ...
           Z_AVG=Z_AVG,$
           NT_BREMS=NT_BREMS, $
           N_E_CHANGE=N_E_CHANGE ;, DELTA_V_VECTOR=DELTA_V_VECTOR

common beam, v_total, cls_b
common beam2,a_c, b_c, denisty, mass, charge, d_length
...
;  of magnitude
        delta_t=abs((min([abs(cls_b)/1d1,(ds[position_index_vol])/1d1]))$
                    /v_parallel_old)
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;Random element Number of electron collisions to proton collisions
        a_c=2d0*randomu(seed)
        b_c=2-a_c
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
        denisty=loop.state.n_e[position_index_vol[0]]
...
TEST:
       Case 1 of
           nt_particles[j].x ge max(s_alt2): begin
               ...
               N_E_CHANGE[INDEX3-loop.n_depth:INDEX3]=$
                 (nt_particles[j].scale_factor/loop.n_depth)$
                 /volumes[INDEX3-loop.n_depth:INDEX3]
               position_index_vol=[INDEX3]
           ...
           nt_particles[j].x le min(s_alt2):  begin
               ...
               delta_e[INDEX3:loop.n_depth]=delta_e[INDEX3:loop.n_depth] $
                              +((nt_particles[j].ke_total) $
                               *nt_particles[j].scale_factor/loop.n_depth)
               nt_particles[j].ke_total=0d0;energy_floor
               ...
               N_E_CHANGE[INDEX3:loop.n_depth]=$
                 (nt_particles[j].scale_factor/loop.n_depth)$
                 /volumes[INDEX3:loop.n_depth]
