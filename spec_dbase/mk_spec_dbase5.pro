...
;                Version 2.0 denotes that now a 3D array is created
;                that 	
;-
ssw_packages, /chianti

wave_min=kev2ang(200d0)
wave_max=400d0
...
FOR t_index=0,n_elements(t)-1 DO BEGIN
    FOR n_e_index=0,n_elements(n_e)-1 DO BEGIN
        ...
        print, 'T='+t_string+', ', 'n_e='+n_e_string
        ...
        isothermal,wave_min,wave_max,wave_step,10.^t[t_index],lambda,spectrum,$
          ...
          ioneq_name=ioneq,/CONT, /all, /noverbose, /photons
        if spectrum[0] ne -1 then begin
           if n_elements(spec_array) eq 0 then begin
              spec_array=dblarr(n_elements(t), n_elements(n_e), n_elements(spectrum)) 
              spec_array[t_index,n_e_index,*]=spectrum
              lambda_old=lambda
              spectrum_old=spectrum
              endif else begin
                 if n_elements(spectrum_old) ne  n_elements(spectrum) then begin
                    spectrum=spline(lambda, spectrum, lambda_old, /double)
                    lambda=lambda_old
                 endif
                 spec_array[t_index,n_e_index,*]=spectrum
              endelse
...
save,lambda, spectrum, spec_array,$
     version,date,units,abundance, ionization,$
     t, n_e, filename=spec_dbase_folder+'/standard_spec_debase.sav'
