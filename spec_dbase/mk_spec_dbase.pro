...
t=dindgen(long(t_max-t_min)/t_step)*t_step+t_min
...
spec_folders=strarr(n_elements(t), n_elements(n_e))
...
FOR t_index=0,n_elements(t)-1 DO BEGIN
    FOR n_e_index=0,n_elements(n_e)-1 DO BEGIN
        ...
        foldername=string(t_folder+'/')
        current_folder=strcompress(parent_folder+$
                  foldername,/remove_all)

        spawn , 'mkdir '+current_folder

        foldername=string(n_e_folder+'/')
        foldername=strcompress(foldername,/remove_all)

        current_folder=strcompress(parent_folder+$
                  foldername,/remove_all)
        
        spec_folders[t_index, n_e_index]=current_folder
...
save, spec_folders, filename,'spec_folders.sav'
