function add_loop_chromo2, loop, T0=T0, DEPTH=DEPTH, N_DEPTH=N_DEPTH, $
...
if not keyword_set(N_DEPTH) then DEPTH=100
...
P=2d0*!msul_kB*[n_e_corona[0]*T_corona[0],$
...
new_s=new_s-new_s[0]
s_alt=msu_get_s_alt(new_s)
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;Gravity
g_add1=-!msul_g0*((!msul_R_Sun/(!msul_R_Sun+z_add1))^2d)
g_add2=!msul_g0*((!msul_R_Sun/(!msul_R_Sun+z_add2))^2d)

;Calculate the gas pressure of a stratified atmosphere
; from the corona down
z_add_alt=msu_get_s_alt(z_add1)
dz=z_add_alt-loop.axis[2,0]
p_chromo1=P[0]*(exp((-1d0*!msul_mp*abs(g_add1)*dz)/(!msul_kB*T0)))

z_add_alt=msu_get_s_alt(z_add2)
dz=z_add_alt-loop.axis[2,n_corona_surf-1]
p_chromo2=P[1]*(exp((-1d0*!msul_mp*abs(g_add2)*dz)/(!msul_kB*T0)))
;Remembering that n_p~n_e and P=nkT
n_e_add1=p_chromo1/(2d0*!msul_kB*T0)
n_e_add2=p_chromo2/(2d0*!msul_kB*T0)
;E=3/2*n*kb*T
E_add1=(3./2.) *2.*n_e_add1*!msul_kB *T0     
E_add2=(3./2.) *2.*n_e_add2* !msul_kB*T0  
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;Summing up
s=new_s
...
b=[dblarr(N_DEPTH_s)+loop.b[0],$
   loop.b,$
   dblarr(N_DEPTH_s)+loop.b[n_corona_surf-1]]
...
;;;;;;;;;;;;;;;;;;;;
;window, 1
;plot,  new_loop.state.e
;window, 2
;plot,  new_loop.state.n_e
;window, 3
;plot,  new_loop.g
