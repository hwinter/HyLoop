...
function add_loop_chromo_tube_constant_pressure, loop, T0=T0, DEPTH=DEPTH, N_DEPTH=N_DEPTH, $
...
;Get values for the corona.
  ...
  too_cool=where(T_corona lt T0, count)
  if count ge 1 then begin
     loop.state.e[too_cool]=(3./2.) *2.*loop.state.n_e[too_cool]*!shrec_kB *T0
     T_corona=get_loop_temp(loop)
  endif

;Pressure at the loop endpoints
  Pressure=2d0*!shrec_kB*[n_e_corona[0]*T_corona[0],$
...
; from the corona down
  ...
  p_chromo1=Pressure[0]*(exp((-1d0*!shrec_mp*abs(g_add1)*dz)/(!shrec_kB*T0)))
  z_add_alt=shrec_get_s_alt(z_add2)
  dz=z_add_alt-loop.axis[2,n_corona_surf-1]
  p_chromo2=Pressure[1]*(exp((-1d0*!shrec_mp*abs(g_add2)*dz)/(!shrec_kB*T0)))
  p_chromo1[*]=.25 ;Mariska et al 1987
  p_chromo2=p_chromo1
...
;Remember no endcaps
  ...
  notes=loop.notes
  notes[0]+= '  . Chromosphere added by add_loop_chromo_tube_constant_pressure :'+string(version)
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
;stop
  ...
  for jj=0ul, 100ul do new_LOOP.state.n_e=smooth(new_LOOP.state.n_e,3)
;plot, s_alt_old, t1
...
;stop
  ...
  print, 'add_loop_chromo_tube_constant_pressure All Done'
;  set_plot, 'x'
 ; stateplot3, new_LOOP, /SCREEN, /CS  
;stop
  return, new_loop
END                             ; Of main
