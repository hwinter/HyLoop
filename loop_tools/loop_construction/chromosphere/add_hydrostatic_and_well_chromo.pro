function add_hydrostatic_and_well_chromo, loop, T0=T0, DEPTH=DEPTH, N_DEPTH=N_DEPTH, $
                                          N_WELL=N_WELL, WELL_DEPTH=WELL_DEPTH, $
                                          VERSION=VERSION, STARTNAME=STARTNAME,$
                                          PERCENT_DIFFERENCE=PERCENT_DIFFERENCE,$
                                          CHROMO_MODEL=CHROMO_MODEL, P_BC=P_BC, $
                                          MIN_STEP=MIN_STEP
Version=1.0
...
if not keyword_set(P_BC) then P_BC=0.25
...
;Number of cells for the constant chromospheric well.
if not keyword_set(N_WELL) then N_WELL=5
;
;Smallest step size we are going to take
if not keyword_set(MIN_STEP) then MIN_STEP=5d4;500 meters or five football fields.
;
case 1 of 
   keyword_set(CHROMO_MODEL) : CHROMO_MODEL_in=CHROMO_MODEL
   not keyword_set(CHROMO_MODEL) and (loop.CHROMO_MODEL eq '') : CHROMO_MODEL_in='CONSTANT CHROMOSPHERE'
   else : CHROMO_MODEL_in='CONSTANT CHROMOSPHERE'
endcase 
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;Estimate minimum depth at each footpoint to go from the coronal base
;pressure to the chromospheric base pressure, assuming constant
;gravity from the corona down.  P[z]=P_0*exp(-mgz/kT) 
min_dz=[-1.0*(!shrec_kB*T0/(g[0]!shrec_mp))*alog(P[0]/P_BC),$
        -1.0*(!shrec_kB*T0/(g[n_corona_surf]!shrec_mp))*alog(P[1]/P_BC)]

test=min(depth-min_dz)
if test lt 0. then begin
   print, 'Depth too short for minimum dz'
   depth=1d6+depth-test   
   print, 'Depth is now '+string(depth)+' [cm]'
endif

if test lt 9.99d5 then begin
   print, 'Depth too short for minimum dz and a 1d6 cm (10km) well'
   depth=1d6+max(min_dz)
   print, 'Depth is now '+string(depth)+' [cm]'
endif
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
old_s=loop.s+DEPTH
...
if A_0*(N_DEPTH_s) le min_dz[0] then begin
    N_DEPTH_s=long(min_dz/A_0)+10l
...
repeat begin
    ...
    y_n=total(d_step1)-(min_dz[0])
    y_prime_n=total((-ind_array/Max(ind_array))*d_step1)
    ;y_prime_n=total(-d_step1)
    
    pd_b=ABS((total(d_step1)-(min_dz[0]))/(min_dz[0]))
    ;PRINT,'PD_B, C1, total(d_step1)/(min_dz[0])', PD_B, C1, total(d_step1)/(min_dz[0]);;
...
d_step1=d_step1*(min_dz[0]/total(d_step1))
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
if A_1*(N_DEPTH_s) le min_dz[1] then begin
    N_DEPTH_s=long(min_dz[1]/A_1)+10l
...
P_BC=2*n_e[0]*T0*!shrec_kB
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
notes=loop.notes
notes[0]+= '  . Chromosphere added by add_hydrostatic_and_well_chromo :'+string(version)
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
print, 'add_hydrostatic_and_well_chromo All Done'
return, new_loop
