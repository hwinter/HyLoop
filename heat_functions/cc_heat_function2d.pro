;+
; NAME:
;	cc_heat_function2d.pro
;
; PURPOSE:
;	Calculate a heating rate that accounts for both particle collisions
;	(using PATC) and heating due to thermal particle
;	collisions. This version of the program requires
;	!injected_beam and !n_beams
;       Replaces cc_heat_function2b.pro
;
; CATEGORY:
;      Heat_function	
;
...
;                    HDWIII 2011-04-27 Modified to work in the new
;                    HyLoop environment.
;-

function cc_heat_function2d, LOOP, time, dt, nt_beam, nt_brems,$
...
;Compute heating due to collisions w/ thermal particles
     e_h =get_nfe_gauss_dist(loop,nano_s , $
                             !heating_rate*(1.0-!FRACTION_PART)/!n_nano,$
                             !nano_fwhm) ;[ergs cm^-3 !s^-1]

     new_beam=!injected_beam[randomu(seed)*(!n_beams-1ul)].nt_beam
     new_beam.x=nano_s[0]
     
     for i=1, !n_nano-1 do begin
         temp_beam=!injected_beam[randomu(seed)*(!n_beams-1ul)].nt_beam
         temp_beam.x=nano_s[i]
         
        new_beam=[new_beam, temp_beam]
     endfor
     Case 1 of    
;First beam case.
