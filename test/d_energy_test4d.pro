...
gif_dir=getenv('DATA')+'/PATC/runs/test/junk/'
PLOT_DIR=gif_dir
spawn, 'mkdir '+gif_dir
gif_prefix='d_energy_test4d'
...
;        window, 0
        ...
        index_string= string(0,FORMAT= $  
                    '(I5.5)')
        POST=PLOT_DIR+'/'+gif_prefix+ $
             '_'+index_string+'.ps'
        gif_files=gif_dir+'/'+gif_prefix+  $
                 '_'+index_string+'.gif'
        print, gif_files
        particle_display, loop,NT_beam,e_min=10, e_max=max(energy[i]),$
                          /XY, post=post
        spawn, 'convert  '+POST+' '+gif_files
;        window, 1
...
;All the particles have been thermalized.  Skip some steps   
        ...
        nt_old=NT_beam
        ;if !d.name eq 'X' then begin
        index_string= string(n_sim_iter,FORMAT= $  
                             '(I5.5)')
        POST=PLOT_DIR+'/'+gif_prefix+ $
             '_'+index_string+'.ps'
        gif_file=gif_dir+'/'+gif_prefix+'_'+  $
                 index_string+'.gif'
        particle_display, loop,NT_beam,e_min=10, e_max=max(energy[i]),$
                          /XY, post=post
...
;Analytic delta_e
        anal_de=de_dt[0]*delta_time;*keV_2_ergs
...
hdw_pretty_plot2, xy0, xy1, xy2, xy3, $
                  ...
                  lthick=2., POSITION=POSITION, NORMAL=1,$
                  YSTYLE=1,yrange=yrange
print, annotation

    image2movie,gif_files, $
                movie_name=patc_dir+gif_prefix+'.mpg',$
                /mpeg,$         ;/java,$
