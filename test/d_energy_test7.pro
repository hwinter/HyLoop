...
delta_time=.0001;1d ;[sec]
loop_time=600.*delta_time;10d*60d
color_table=39
...
;  The NRL plasma formulary
        debye_length=6.9010323*((T_plasma[0])/ $
                                (loop.state.n_e[0]))^0.5d
        V=energy2vel(nt_old.ke_total)
        VP=V*sin(nt_old.pitch_angle)
        e_n_vp=[[[nt_old.ke_total]],[[vp]]]
        help, e_n_vp[0], debye_length, $
              loop.state.n_e[0] ,nt_old.mass,nt_old.charge
        
        ln_lambda=get_loop_coulomb_log( loop)
        print, 'ln lambda= ',ln_lambda[0]
        
        de_dt=patc_de_dt(0, e_n_vp,debye_length ,$
                          loop.state.n_e[0] ,$
                         nt_old.mass,nt_old.charge,$
                         1.,0.0, Coul_out=Coul_out)
 ;       ln_lambda=Coul_out[0]/2
 ;       c=(4d0) $
 ;         *!dpi $
 ;         *loop.state.n_e[0] $
 ;         *(!msul_qe^4.0)$
 ...
 ;           /($
 ;             (sqrt(!msul_me^3.0)) $
 ;             *sqrt(2.0*nt_old.ke_total*!msul_keV_2_ergs)$
 ;         )
 ...
 ;           !msul_me*v $
 ;           )
        c=(2d0)*!dpi*(!msul_qe^4.0)*ln_lambda[0]
        c=c/(nt_old.ke_total*!msul_keV_2_ergs)
        c=c*(2d0)*loop.state.n_e[0]*v
       

;e_f=($
;     (3.0/2.0)*sqrt(!msul_me/2.0) $
;     *(C)* (2d0)*loop.state.n_e[0] $
;     *delta_time $
;     +((nt_old.ke_total*!msul_keV_2_ergs)^(3./2.)) $
;    )^(2/3)
...
;Analytic delta_e
        anal_de1=de_dt[0]*delta_time;*keV_2_ergs
        anal_de=delta_time*(-C) $
        /!msul_keV_2_ergs
        
        mu_rm=(nt_old.mass*!msul_me)/(!msul_me+nt_old.mass)
        si=!msul_qe*!msul_qe/(mu_rm*v*v)
      ;  anal_de=delta_time*$
      ;          (-1d0)*$
      ;          (2d0)*!dpi*v*v*v*(mu_rm*mu_rm/!msul_me) * $
      ;          si*si*loop.state.n_e[0]*alog(debye_length*debye_length/(si*si));ln_lambda[0]
        print, 'anal_de= ',anal_de
        print, 'anal_de1= ',anal_de1
        print, 'delta_e1',delta_e1
       ; stop
        etest_temp=100.*((anal_de-delta_e1)/anal_de)
        etest_temp=anal_de/delta_e1
        if n_elements(anal_de) gt 1 then $
          avg_de1=moment(anal_de) else $
          avg_de1=anal_de
        de1=[de1,$
             avg_de1[0]]
        e_array=[  e_array,etest_temp]
        print, 'Avg % diff:'+string(etest_temp)
        print, ';;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;'
        if !d.name eq 'X' then begin
            window, 0
            particle_display, loop,NT_beam,e_min=10, e_max=max(energy[i]),$
...
;plot,BEAM_STRUCT.time, etest, psym=5,XTITLE='Time'
    ...
    e_array=e_array[1:n_elements(de2)-1]
    pmm, etest
    percent_diff[i,0:n_elements(e_array)-1]=e_array
;pmm,etest2
...
for i=0, n_energies-1 do begin
    ...
    ymin<=min(percent_diff[i,*])
endfor
n_b_max=n_elements(de1)
n_b_max<=500
...
for i=0, n_energies-1 do begin
    ...
    annotation=[annotation,$
                ...
                ' Avg Ratio.='+string(mean[0], format=format)]
    
endfor
max_ind=n_elements(n_steps[0,*])-2ul
annotation=annotation[1:*]
xy0=reform([[[n_steps[0,0:max_ind]]],[[percent_diff[0,0:max_ind]]]])
xy1=reform([[[n_steps[1,0:max_ind]]],[[percent_diff[1,0:max_ind]]]])
xy2=reform([[[n_steps[2,0:max_ind]]],[[percent_diff[2,0:max_ind]]]])
xy3=reform([[[n_steps[3,0:max_ind]]],[[percent_diff[3,0:max_ind]]]])
help, xy0, xy1


yrange=[1.0,ymax*1.33]
print, 'wrote file: '+EPS_name
POSITION=[0.20, 0.15, .95, .9]
hdw_pretty_plot2, xy0, xy1, xy2, xy3, $
                  label=annotation, left=1,top=1, box=0,$
                  YTICKFORMAT=format, TITLE='Energy Test 2',$
                  XTITLE='N Timesteps',YTITLE='Analytic / Numeric',$
                  EPS=EPS_name, charsize=1.2, charthick=1.2,$
                  lcharsize=1.1, lcharthick=1.1,$
                  lthick=2,thick=2, POSITION=POSITION, NORMAL=1,$
