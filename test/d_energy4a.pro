...
e_n_vp=[[[nt_old.ke_total]],[[0]]]
de_dt=patc_de_dt( 0 , e_n_vp,debye_length ,loop.state.n_e[0] ,m,c)
...
;Analytic delta_e
        ...
        anal_de=-A_factor*v_total*delta_time*(((mu_e/!msul_me) $
                                               *xi_e) )
        etest_temp=average((anal_de-delta_e1)/anal_de)
        if n_elements(anal_de) gt 1 then $
          avg_de1=moment(anal_de/kev_2_ergs) else $
          avg_de1=anal_de/kev_2_ergs
        de1=[de1,$
             avg_de1[0]]
        e_array=[  e_array,average(((delta_e1-anal_de)/anal_de))]
        print, 'Avg % diff:'+string(average(((delta_e1-anal_de)/anal_de)))
        print, ';;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;'
        if !d.name eq 'X' then begin
            window, 0
            particle_display, loop,NT_beam,e_min=10, e_max=max(energy[i]),$
