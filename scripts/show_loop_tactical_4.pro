;show_loop_tactical_script_4
Data_dir=getenv('DATA')+'/PATC/runs/'
movie_name='temperature'
...
if files1_count le 0 then goto, end_loop
    ...
    for files_index=0UL, FILES1_COUNT-2ul do begin
        ...
        max_e_dep>=max((get_loop_temp(loop)))
        min_e_dep<=min((get_loop_temp(loop)))
    endfor    
    restore, files1[0]
    help, loop
;stop
...
;Rad tag was properly set in all versions.
        ...
        fill=(get_loop_temp(loop))*e_color;+bottom_color
      ;  window, 3
