...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
function shrec_conduction, state, s,a, $
...
K2 = shrec_kappa(S_GRID_TEMP,SO=SO) 
...
; Matte & Virmont, 1982   
    ...
    L_T=shrec_get_temp_cls(state,s_alt, floor=1d-5,$
                       /S_GRID)       ;, /smooth)    
    ;pmm, l_t
   ; L_T>=1d-12

    mfp=shrec_get_mfp( state,s,T=TEMP,/S_GRID,$
...
;    F_sat=(1./4.)*n_e_s_grid[iii]*0.5*!shrec_me $
;          *((3.*!shrec_kB*S_GRID_TEMP[iii]/!shrec_me)^1.5d0)
;    ratio=F_c/F_sat
;    ratio>=1d0
;    F_c/=ratio
;stop
